...
| **Ordini / Cassa** | POS (tablet/telefono), pagamenti, **comande** verso la cucina con schermi per sotto-reparto (KDS). Comande e cassa sono due facce dello stesso "ordine" → **un modulo solo**, non due che si coordinano | aree + **sotto-reparti**, layer metriche | ⚠️ **Fiscalità italiana** (corrispettivi, scontrino elettronico, ETS/terzo settore): la parte più regolamentata. **Slice A (fatto)**: listino per evento (prodotti, prezzo, area/sotto-reparto), POS con carrello e totale, contante + "segna pagato", numero ordine progressivo per evento, righe con snapshot del prodotto. Listino = organizzatore, cassa = staff. **Punti cassa (fatto)**: la *cassa* è un'entità che **appartiene a un'area** — così regge sia la sagra con un reparto "Cassa" dedicato (il suo responsabile configura tutto, e ha turni propri come ogni area) sia quella dove **ogni reparto si autogestisce** la propria; chi configura una cassa è il responsabile della sua area (o l'organizzatore), riusando `managesArea()` senza nuovi concetti di permesso. Più casse possono avere **menù diversi**; menù vuoto = vende tutto il listino, così chi ha una cassa sola non configura niente. Due assi ortogonali: `prodotto.area` = *chi prepara* (instrada le comande), `cassa.area` = *chi vende e configura*. I **prodotti** restano dell'organizzatore. **Slice B (fatto)**: comande/KDS — ogni voce d'ordine ha uno stato (`pending → ready → served`) e la pagina Comande è uno schermo per area, restringibile a un sotto-reparto, con auto-refresh; le voci senza area nascono già servite. **Poi**: pagamenti elettronici (SumUp/Satispay/Nexi) e scontrino |
