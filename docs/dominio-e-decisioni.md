...
| **Forniture** | Contatti fornitori per reparto/sotto-reparto, acquisto/noleggio/prestito consumabili e attrezzature, storico con upload fatture/note | aree + sotto-reparti, storage documenti | Basso rischio, valore chiaro. **Primo modulo "vero" dopo i turni — in sviluppo.** Slice 1 (fatto): anagrafica fornitori tenant-level + forniture per evento/area/sotto-reparto (tipo, costo, data, note), accesso organizzatore + responsabili scoped. **Fase 2**: upload allegati (fatture/note) con storage privato. Struttura Laravel standard, nessuna cartella `Modules/` (D13) |
