...
// Direct assignment ("ti metto io"): an existing person, or a new one.
function assignExisting(id: number) {
...
<template>
    <div v-if="signups.length > 0 || people" class="mt-2 grid gap-1 border-t pt-2">
        ...
        <PersonPicker
            v-if="people"
            :people="people"
            :exclude="signups.map((s) => s.personId)"
            label="Metti una persona"
            title="Metti una persona sul turno"
            @pick="assignExisting"
            @create="assignNew"
        />
    </div>
